...
@Slf4j
@Component
public class SessionQueuedEventListener implements ChatEventListener {
    ...
    public boolean supports(ChatEvent event) {
        return ChatEventType.SESSION_QUEUED == event.getEventType();
    }
    ...
    public void handleEvent(ChatEvent event) {
        ...
        log.info("Handling session queued event: {}", event);
